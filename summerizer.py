...
def summerizer(
    ...
    """
    _logger.info(f'analyzing folder: {data_folder} with output directory: {out_dir}, format: {out_fmt}, key: {eval_key}, type: {eval_type} and metrics: {metrics}.')
    ...
    fname_pattern = re.compile('ev_dump_*.csv')
    for r, d, f in os.walk(data_folder):
        for file in f:
            if fname_pattern.fullmatch(file):
                fullpath = os.path.join(r, f)
                mycounter.update_by_file(fullpath, header=True)

    # Calculate the metrics
    mycalc = Metric(my_kvstore)
    res = mycalc.calc_stats(eval_key=eval_key, eval_type=eval_type, metrics=list_of_metrics)
    ...
    if not res:
        ...
        if out_fmt == 'csv':
            ...
            try:
                ...
                _logger.error('Failed to write the result to diretory: {}, error: {}' % (target_path, e))
        elif out_fmt == 'json':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', help='the output directory to store analysis results.')
    parser.add_argument('--output_fmt', help='the format of analysis results. Default CSV.')
    ...
    parser.add_argument('--metrics', help='the list of metrics to be calculated. Represented by a joined string separated by commas.')
